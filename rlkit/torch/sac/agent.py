...
#        self.clear_z()
    ...
    def get_action(self, obs, deterministic=False):
        ...
        return self.meta_policy.get_action(in_, deterministic=deterministic)

    def set_num_steps_total(self, n):
        self.meta_policy.set_num_steps_total(n)
    ...
    def forward(self, obs, task_indices=None):
        ...
        # run policy, get log probs and new actions
        in_ = torch.cat([obs], dim=1)
        meta_policy_outputs = self.meta_policy(t, b, in_, reparameterize=True, return_log_prob=True)

        # if not self.use_ib:
        #     task_z_vars = [z.repeat(b, 1) for z in self.z_vars]
        #     task_z_vars = torch.cat(task_z_vars, dim=0)
        #     return policy_outputs, task_z, task_z_vars
        return meta_policy_outputs
