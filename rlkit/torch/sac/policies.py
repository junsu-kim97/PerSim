...
class TanhGaussianPolicy_ours(Mlp, ExplorationPolicy):
    """
    Usage:
    ...
    def __init__(
        ...
        if std is None:
            ...
            self.last_fc_log_std = nn.Linear(last_hidden_size, np.prod(action_dim))
    ...
    def forward(
        ...
        for i, fc in enumerate(self.fcs):
            fc.to(ptu.device).requires_grad_()
        ...
        self.last_fc.to(ptu.device).requires_grad_()

        self.last_fc_log_std.to(ptu.device).requires_grad_()
