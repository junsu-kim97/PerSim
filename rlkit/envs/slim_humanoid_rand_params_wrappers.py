import numpy as np
import json
from rand_param_envs.slim_humanoid_rand_params import SlimHumanoidRandParamsEnv
...
def read_log_params_new(log_file):
    with open(log_file) as f:
        js = json.load(f)
    params_dict = {}
    for key, value in js.items():
        if isinstance(value, list):
            params_dict[key] = np.asarray(value)
        else:
            params_dict[key] = value
    return params_dict


@register_env("slim-humanoid-rand-params")
class SlimHumanoidRandParamsWrappedEnv(SlimHumanoidRandParamsEnv):
    def __init__(self, n_tasks=2, randomize_tasks=True, max_episode_steps=200):
        super(SlimHumanoidRandParamsWrappedEnv, self).__init__()
    ...
    def sample_tasks(self, n_tasks):
        ...
        if self.randomize_tasks:
            ...
            for i in range(n_tasks):
                ...
                task_params = read_log_params_new(f"/home/changyeon/CaDM_MerPO/data/slim-humanoid-rand-params/goal_idx{i}/log.txt")
...
@register_env("sparse-slim-humanoid-rand-params")
class SparseSlimHumanoidRandParamsWrappedEnv(SlimHumanoidRandParamsWrappedEnv):
    def __init__(self, n_tasks=2, randomize_tasks=True, max_episode_steps=1000, goal_radius=0.5):
        super(SparseSlimHumanoidRandParamsWrappedEnv, self).__init__(n_tasks, randomize_tasks, max_episode_steps)
