...
class SimpleReplayBuffer(ReplayBuffer):
    ...
    def add_sample_batch(self, observations, actions, rewards, terminals,
                   next_observations, batch_size,**kwargs):
        for i in range(0,batch_size):
            self._observations[self._top] = observations[:,i,:]
            self._actions[self._top] = actions[:,i,:]
            self._rewards[self._top] = rewards[:,i,:]
            self._terminals[self._top] = terminals[:,i,:]
            self._next_obs[self._top] = next_observations[:,i,:]

            # if rewards[:,i,:] >= self.goal_radius:
            #     sparse_reward = (rewards[:,i,:] - self.goal_radius) * (1/abs(self.goal_radius))
            #     self._sparse_rewards[self._top] = kwargs['env_info'].get('sparse_reward', sparse_reward)
            # else:
            #     self._sparse_rewards[self._top] = kwargs['env_info'].get('sparse_reward', 0)
            self._advance()
    ...
    def random_batch(self, batch_size):
        ...
        assert self._size > 0, f"buffer size: {self._size}"
